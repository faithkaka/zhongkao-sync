...
const app = {
    ...
    checkSupabaseConfig() {
        if (typeof window.SUPABASE_CONFIG === 'undefined' || !window.SUPABASE_CONFIG.url) {
            ...
            console.log('✅ Supabase 配置已加载:', window.SUPABASE_CONFIG.url);
        }
    },
            ...
            if (!document.querySelector('.config-warning')) {
                ...
                warning.innerHTML = `
                    <div class="warning-content">
                        ...
                        <span class="warning-text">数据服务未配置</span>
                        <button class="warning-btn" onclick="document.querySelector('.config-warning').remove()">✕</button>
                    </div>
                `;
    ...
    async checkUserLogin() {
        ...
        console.log('Users:', users.length, 'Current:', currentUserId);
    ...
    async updateUserDisplay() {
        ...
        try {
            ...
            console.log('✅ 用户显示更新:', userNameEl.textContent);
        } catch (error) {
    ...
    bindEvents() {
        document.querySelectorAll('.semester-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                console.log('📅 学期选择:', this.semesterNames[this.state.selectedSemester]);
            });
        });
    ...
    renderQuestion() {
        ...
        if (optionsEl) {
            optionsEl.innerHTML = question.options.map((opt, idx) => 
                '<button class="option" data-idx="' + idx + '" onclick="app.selectOption(' + idx + ')">' + 
    ...
    calculateSubjectStats() {
        ...
        this.state.currentQuestions.forEach((q, idx) => {
            ...
            if (this.state.userAnswers[idx] && this.state.userAnswers[idx].correct) {
    ...
    async renderUserList() {
        ...
        container.innerHTML = users.map(user => {
            ...
            return '<div class="user-item ' + (isCurrent ? 'current' : '') + '">' +
                '<div class="user-info">' +
                ...
                (isCurrent ? '<span class="user-current-badge">当前</span>' : '<button class="user-btn switch" onclick="app.switchUser(\'' + user.id + '\')">切换</button>') +
